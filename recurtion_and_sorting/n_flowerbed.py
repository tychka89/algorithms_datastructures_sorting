...
Пример 1: Два одинаковых отрезка [7, 8] и [7, 8] сливаются в один, но потом их накрывает отрезок [6, 10].
Таким образом, имеем две клумбы с координатами [2,3] и [6,10].

Пример 2: Отрезки [2,3], [3, 4] и [3,4] сольются в один отрезок [2,4].
...
def flower_bed(data):
    ...
    for i in range(n - 1):
        if end < data[i + 1][0]:
            data_new.append('{} {}'.format(start, end))
            start = data[i + 1][0]
            end = data[i + 1][1]
        elif data[i + 1][1] > end:
            end = data[i + 1][1]
